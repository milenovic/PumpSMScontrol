...
////#endif
     ...
     */
    private TypeOfAddress smscTypeOfAddress;
     ...
     */
    private TypeOfAddress senderTypeOfAddress;
    /**
     ...
     */
    private class DataCodingScheme {
    ...
    private class TimeStamp { //The TP-Service-Centre-Time-Stamp field is given in semi-octet representation (each semi-octet consists of two digital decimals), and represents the local time in the following way:
    ...
    private class UserData {
        ...
        class UDH {
            ...
            String cropUserData(String userDataHex, DataCodingScheme DCS, FirstOctet firstOctet, int udhLength, int UDL) {
                if (DEBUG) {
                    System.out.println("Start cropUserData: userDataHex=" + userDataHex + ", DCS=" + DCS + ", UDL=" + UDL);
                }
                if (DCS.alphabet == DataCodingScheme.Alphabet.GSM) {
                    if (DEBUG) {
                        System.out.println("Alphabet is GSM");
                    }
                    int skipBits = 0;
                    ...
                    if (DEBUG) {
                        System.out.println("Alphabet is UCS2");
                    }
                    if (firstOctet.TP_UDHI) {
                    ...
                    if (DEBUG) {
                        System.out.println("Alphabet is Unknown");
                    }
                    if (firstOctet.TP_UDHI) {
                        return ATStringConverter.UCS2Hex2Java(userDataHex.substring((udhLength + 1) * 2));
                    } else {
    ...
    private String hexInternalSwap(String hex) {
        if (DEBUG) {
            System.out.println("Starting hexInternalSwap: " + hex);
        }
        if (hex.length() % 2 == 1) {
            if (DEBUG) {
                System.out.println("ERROR: number of characters is odd");
            }
            return null;
        }
        String result = "";
    ...
    private String fromHexStringToAlphanumeric(String hexString, int numberOfUsefulSeptets, int skipBits) {
        if (DEBUG) {
            System.out.println("Starting fromHexStringToAlphanumeric: hexString=" + hexString + ", numberOfUsefulSeptets=" + numberOfUsefulSeptets + ", skipBits=" + skipBits);
        }
        boolean[] udBool = new boolean[(hexString.length() / 2) * 8]; //User data presented in binary array
        if (DEBUG) {
            System.out.println("udBool.length=" + udBool.length);
        }
        ...
        if (DEBUG) {
            System.out.println("septets.length=" + septets.length);
        }
        for (int i = skipBits / 7; i < numberOfUsefulSeptets; i++) { //Fill the septets array with portions of 7 bits from udBool, converted to byte
            ...
            if (DEBUG) {
                System.out.println("septets[" + (i - (skipBits / 7)) + "]=" + septets[i - (skipBits / 7)]);
            }
        }
...
//            System.out.println("Starting to parse PDU: " + hexString);
        ...
        if (DEBUG) {
            System.out.println("senderTypeOfAddress: " + senderTypeOfAddress);
        }
        index += 2;
...
//            System.out.println("senderNumber: " + senderNumber);
        ...
        if (DEBUG) {
            System.out.println("protocolIdentifier: " + protocolIdentifier);
        }
        index += 2;
        dataCodingScheme = new DataCodingScheme(hexStringPDU.substring(index, index + 2));
        if (DEBUG) {
            System.out.println("dataCodingScheme: " + dataCodingScheme);
        }
        index += 2;
